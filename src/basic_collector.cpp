...
XPTI_CALLBACK_API void xptiTraceInit(unsigned int major_version,
  ...
  if (stream_name) {
    ...
    xptiRegisterCallback(
        GStreamID, (uint16_t)xpti::trace_point_type_t::transfer_begin, tpCallback);
    xptiRegisterCallback(
        GStreamID, (uint16_t)xpti::trace_point_type_t::transfer_end, tpCallback);

    xptiRegisterCallback(
        GStreamID, (uint16_t)xpti::trace_point_type_t::signal, tpCallback);
...
XPTI_CALLBACK_API void tpCallback(uint16_t TraceType,
  ...
  uint64_t pID = Parent ? Parent->unique_id : 0;
  // Lock while we print information
  std::lock_guard<std::mutex> Lock(GIOMutex);
  // Print the record information
  if (TraceType == (uint16_t)xpti::trace_point_type_t::node_create) {
    printf("----------------- node_create ------------- event_id=%10lu --- p_ev_id=%10lu | ", ID, pID);
  }
  if (TraceType == (uint16_t)xpti::trace_point_type_t::edge_create) {
    printf("----------------- edge_create ------------- event_id=%10lu --- s=%10lu --- t=%10lu | ", ID,
      Event->source_id, Event->target_id);
  }
  if (TraceType == (uint16_t)xpti::trace_point_type_t::task_begin) {
    printf("----------------- task_begin ------------- event_id=%10lu --- p_ev_id=%10lu | ", ID, pID);
  }
  if (TraceType == (uint16_t)xpti::trace_point_type_t::task_end) {
    printf("----------------- task_end ------------- event_id=%10lu --- p_ev_id=%10lu | ", ID, pID);
  }

  if (TraceType == (uint16_t)xpti::trace_point_type_t::transfer_begin) {
    printf("----------------- transfer_begin ------------- event_id=%10lu --- s=%10lu --- t=%10lu | ", ID, Event->source_id, Event->target_id);
  }
  if (TraceType == (uint16_t)xpti::trace_point_type_t::transfer_end) {
    printf("----------------- transfer_end ------------- event_id=%10lu --- s=%10lu --- t=%10lu | ", ID, Event->source_id, Event->target_id);
  }
  if (TraceType == (uint16_t)xpti::trace_point_type_t::signal) {
    printf("----------------- signal ------------- event_id=%10lu --- s=%10lu --- t=%10lu | ", ID, Event->source_id, Event->target_id);
  }
  printf("name=%-35s\n", Name.c_str());
